...
export const KlyvoraLogo: React.FC<KlyvoraLogoProps> = ({
  ...
  const iconSize = size === 'lg' ? 'w-12 h-12' : size === 'md' ? 'w-8 h-8' : 'w-[28px] h-[28px]';
  return (
    <div className={`flex items-center gap-2 select-none ${className}`}>
      {/* Icon: Small dark purple square with subtle play & spark */}
      <div className={`${iconSize} rounded-lg bg-gradient-to-br from-[#5B3FD6] to-[#8B5CF6] p-[1px] shadow-sm flex items-center justify-center`}>
