...
async function startServer() {
  ...
  app.post('/api/preview-voice', async (req, res) => {
    try {
      const { voice = 'beta-free', language = 'Português do Brasil' } = req.body || {};
      const mp3Buffer = await synthesizeFreeTts(voicePreviewText(language), language);
  ...
  app.post('/api/generate-narration', async (req, res) => {
    ...
    try {
      const { video_id, voice_id, script: clientScript, language: clientLanguage } = req.body || {};
      ...
      if (isSupabaseMode) {
        ...
        if (video_id) {
          ...
          if (!vData || vData.user_id !== userId) return res.status(404).json({ error: 'Video record not found.' });
          videoRecord = vData;
          if (vData.series_id) {
      ...
      if (!scriptText) return res.status(400).json({ error: 'Roteiro ausente para gerar narração.' });
      const language = seriesRecord?.language || clientLanguage || 'Português do Brasil';
      const voiceName = voice_id || seriesRecord?.voice_id || 'beta-free';
      const mp3Buffer = await synthesizeFreeTts(scriptText, language);
      let narrationUrl = `data:audio/mpeg;base64,${mp3Buffer.toString('base64')}`;
      let storagePath: string | null = null;

      if (isSupabaseMode && supabaseAdminClient && video_id) {
        storagePath = `${userId}/videos/${video_id}/narration.mp3`;
        const { error: uploadErr } = await supabaseAdminClient.storage.from('media').upload(storagePath, mp3Buffer, { contentType: 'audio/mpeg', upsert: true });
        if (!uploadErr) {
      ...
      return res.status(200).json({
        ...
        narration_voice: voiceName,
        narration_status: 'ready',
        narration_duration: Math.max(5, Math.round(String(scriptText).split(/\s+/).filter(Boolean).length / 2.5)),
        provider: 'google-translate-tts-beta',
      });
    } catch (err: any) {
      console.error('Error in /api/generate-narration:', err);
      if (targetVideoId && supabaseAdminClient) {
        try { await supabaseAdminClient.from('videos').update({ narration_status: 'failed' }).eq('id', targetVideoId); } catch (_) {}
      }
  ...
  // Uses @cf/black-forest-labs/flux-1-schnell via server-lib/cloudflareImages.ts
  // =========================================================
    ...
    try {
      ...
      if (isSupabaseMode && supabaseAdmin) {
        ...
        sceneRecord = req.body?.scene || {
          ...
          user_id: userId,
          scene_order: 1,
          visual_prompt: 'A cinematic scene for a faceless video',
        };
      }

      const finalPrompt = buildEnrichedVisualPrompt(sceneRecord.visual_prompt, seriesRecord);
      const { base64: imageBase64, mimeType } = await generateCloudflareImage(finalPrompt);
      ...
      return res.status(500).json({ error: err.message || 'Falha ao gerar imagem da cena com Cloudflare Workers AI.' });
    }
  });
    ...
    try {
      ...
      for (let i = 0; i < scenes.length; i++) {
        ...
        try {
          ...
          const { base64: imageBase64, mimeType } = await generateCloudflareImage(prompt);
          ...
          if (isSupabaseMode && supabaseAdmin) {
            storagePath = `${userId}/videos/${video_id}/scenes/${scene.id}.jpg`;
